...
func (s *Server) Run(ctx context.Context) error {
	...
	sub, err := nc.Subscribe(tunnel.DownSubject(s.cfg.Tunnel.SubjectPrefix), s.handleDownstream)
	if err != nil {
...
func (s *Server) serveClient(ctx context.Context, conn net.Conn) {
	...
	if err := s.registry.Add(sessionID, client); err != nil {
		s.logger.Printf("[%016x] register session failed: %v", sessionID, err)
	...
	if err != nil {
		s.logger.Printf("[%016x] encode connect request failed: %v", sessionID, err)
		_ = tunnel.FailureReply(conn)
		return
	}
	...
	if err != nil {
		s.logger.Printf("[%016x] exit connect request failed: %v", sessionID, err)
		_ = tunnel.FailureReply(conn)
		return
	}

	if string(response.Data) != "OK" {
		s.logger.Printf("[%016x] exit refused connection to %s:%d: %s", sessionID, addr.Host, addr.Port, strings.TrimSpace(string(response.Data)))
		_ = tunnel.FailureReply(conn)
		return
	}
	...
	s.logger.Printf("[%016x] proxying %s:%d", sessionID, addr.Host, addr.Port)
	upSubject := tunnel.UpSubject(s.cfg.Tunnel.SubjectPrefix)
